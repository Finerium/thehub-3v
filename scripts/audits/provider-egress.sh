...
# scripts/models/fetch.ts (the build-time model download under a hash check, ADR-009), scripts/golden/** (the
# golden runner, which drives the deployment under test over its own HTTP routes), src/contracts/generated/**
...
while IFS= read -r f; do
  [ -z "$f" ] && continue
  case "$f" in
    src/gateway/*|scripts/models/fetch.ts|scripts/bundle/pull.ts|scripts/golden/*|tests/*|*.test.ts|*.test.tsx|*.spec.ts) continue ;;
  esac
