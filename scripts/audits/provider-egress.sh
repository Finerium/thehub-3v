...
# scripts/models/fetch.ts (the build-time model download under a hash check, ADR-009), scripts/golden/** and scripts/export/** (the
# golden runner and the export builder, which drive this application over its own HTTP routes), src/contracts/generated/**
...
while IFS= read -r f; do
  [ -z "$f" ] && continue
  case "$f" in
    src/gateway/*|scripts/models/fetch.ts|scripts/bundle/pull.ts|scripts/golden/*|scripts/export/*|tests/*|*.test.ts|*.test.tsx|*.spec.ts) continue ;;
  esac
