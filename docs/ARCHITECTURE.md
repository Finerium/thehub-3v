...
src/app/
  ...
  (hub)/demo/loop/page.tsx                         11 Guided loop (sandbox per browser, D-16)
...
drizzle/                SQL migrations generated by drizzle-kit (drizzle.config.ts), applied with the unpooled owner URL
src/db/                 schema.ts (Drizzle), seed/ (bundle loader), client.ts (pooled URL of the `thehub_app` role, D-20)
...
bundle/                 the pulled harness release (section 2); chunks.jsonl, text/, pages/, opls.json untracked (.gitignore, D-17)
recordings/<case_id>/<role>.json   Tier B replay recordings (9.16), dehydrated (section 9.4)
scripts/                contracts/generate.ts, bundle/pull.ts, db/{migrate,seed,reset,app-role,retention}.ts (D-20), ag1/extract.ts (D-18),
                        seeded-version-id.ts, golden/run.ts, export/build.ts, smoke.ts, audits/*.sh (deterministic checks, run in CI),
                        api-reference.ts (writes docs/api.md from the routes and the contracts; --check fails on drift)
...
docs/                   ARCHITECTURE.md (this), adr/ADR-001..013.md, DISCLOSURE-AI.md (D-04), runbook.md (seed, activation, rotation, recovery), api.md (generated)
...
.env.example, vercel.json, LICENSE, CHANGELOG.md, README.md, Report.md (end of run)
```

Package scripts: `gate:quick` = eslint + `tsc --noEmit` + vitest; `test:e2e`; `contracts:check` (regenerate, expect no diff, validate every bundle file against both JSON Schema and Zod); `bundle:pull`; `db:migrate` (unpooled URL), `db:app-role` (unpooled URL; creates and grants the `thehub_app` role, D-20), `db:seed`, `db:reset` (CI and recovery only), `db:retention` (unpooled URL; the nightly job's step of section 3.3); `ag1:extract` (the extractor role over the gateway, D-18, section 9.1); `export:demo`; `deck:build`; `video:capture`, `video:encode`; `golden:a`, `golden:b`, `golden:b:replay`; `smoke`; `docs:api` (write `docs/api.md`; `pnpm docs:api --check` regenerates and fails on a diff).
...
harness/       analyze_corpus.py (fixtures), pdftext.py (the one extractor call, `pdftotext -raw`), canonical.py, workbook.py,
               ...
               g1.py, bundle.py (writer + manifest); no ag1.py and no provider client (AG-1 is `pnpm ag1:extract` in thehub-3v, D-18)
packages/      tracked package sources (fixtures.json, chains.json, families.json, pid_sidecars/, spot pins, labels, aliases)
golden/cases.yaml, rulepack/v1.json, contracts/ (JSON Schema 2020-12, the ONE home of every schema: 16 files, section 1.4)
...
bundle/        generated, untracked; the release tarball excludes chunks.jsonl, text/, pages/ and opls.json (D-17)
```
...
`thehub-harness/contracts/**/*.schema.json` is the single source. Python validates package artefacts with `jsonschema`; `thehub-3v/scripts/contracts/generate.ts` runs `json-schema-to-zod` (devDependency, generation time only) into `src/contracts/`, committed; `pnpm contracts:check` regenerates and fails on a diff, then validates every bundle file with both validators. Drizzle `src/db/schema.ts` pins the same enums by importing the Zod enums. Sixteen schema files, several 9.x types each: `bundle/manifest.schema.json`; `entities/document.schema.json` (9.2), `entities/asset.schema.json` (9.3), `entities/operations.schema.json` (9.4), `entities/coverage.schema.json` (9.5), `entities/drafts.schema.json` (9.6), `entities/serving.schema.json` (9.7 corpus version, people, audit, evaluation); `evidence_packet.schema.json` (9.8), `rulepack.schema.json` (9.10), `golden_case.schema.json` (9.11), `team_facts.schema.json` (9.12), `gateway.schema.json` (9.13), `fixtures.schema.json` (10.5); `connectors/edms.schema.json`, `aims.schema.json`, `historian.schema.json` (9.14, served byte for byte by `/api/connectors/:name/schema`). `contracts/bundle_map.json` maps every 9.1 file to its $def; `contracts/CHECK.md` records the review.

The reader-facing end of the same flow is [`docs/api.md`](api.md), the API reference (AC-NFR-25). It is generated by `scripts/api-reference.ts` (`pnpm docs:api`) from the route files under `src/app/api`, the matrix of 9.9 in `src/auth/matrix.ts`, the typed error states in `src/lib/errors.ts` and the Zod modules above: every route with its methods, the permission it asks for and the roles that hold it, its path, query and body schemas as the route declares them, and every status it answers with. Nothing in it is written by hand, and `pnpm docs:api --check` regenerates it and exits non-zero when the file and the code disagree, so a route that gains a method or changes its column cannot leave a stale reference behind.
...
| bundle | same | `make bundle` | `bundle/` per 9.1 with `manifest.json` (every file's sha256, `extractor`, `recipe_sha256`, `stop_list_sha256`, `rulepack_version`, `embedding_model: null`); `opls.json` is corpus text (sections 1 to 6 of every lesson), so it is written here, where the corpus exists, and listed in the manifest with its sha256 (D-17) |
| release | build machine | `make release` (`gh release create v<semver>`) | `thehub-bundle-<semver>.tar.gz` = the bundle minus `chunks.jsonl`, `text/`, `pages/` and `opls.json` (D-17); the manifest still lists them (path, sha256, bytes) so G1 can verify the locally produced copies |
| pull | `thehub-3v` | `pnpm bundle:pull <semver>` | `bundle/` tracked public parts (never `opls.json`, gitignored in both public repositories, D-17); the manifest is the pin (`bundle/manifest.json`) |
| G1 (TS) | `src/gates/g1.ts` | `pnpm db:seed --bundle bundle/` | admits or names the violation |
| seed | where `chunks.jsonl`, `pages/` and `opls.json` exist (build machine for production Neon; CI service container for AC-ING-15) | same | deterministic rows; corpus version v1 |

G1 in TypeScript checks exactly the 9.1 list: every listed file's SHA-256 (the seed-time files `chunks.jsonl`, `text/`, `pages/` and `opls.json` included, which the release omits and the manifest still lists, D-17), every file against its Zod schema, fixture counts (98 files, 56 lessons, 211 work orders, register total 174), referential closure (every `span_id`, `document_id`, `document_revision_id`, `wo_number`, `opl_id`, `bom_item_id` resolves inside the bundle), closed-set membership (every enum and every `entity_binding` in its candidate set), and every quoted span's hash recomputed as `sha256(canonical(anchor_text))` from the bundle's own text (`claims.json` anchors and `chunks.jsonl` text); the Python `harness/g1.py` recomputes the same hashes from the extracted page text. Four mutation tests (AC-ING-09) are the gate's specification.
...
| `integrity_finding` | `id` | `rule_id`, `rule`, `severity`, `discipline` nullable, `document_id` FK nullable (the CD-16 area observations bind to no single document), `span_id` FK nullable, `observation_only` (CD-15 and CD-16 sit outside the 174 total), `unit`, `basis`, `item` jsonb, `state` (`open` or `resolved`, section 13 decision), `safety_function` boolean, `routing_recommendation` nullable, `corpus_version_id`; totals pinned to `fixtures.json` (AC-INT-01) |
...
| `sandbox` | `id` | `corpus_version_id` FK nullable, `created_at`, `last_seen_at`; one row per visitor browser, named by the `thehub_sandbox` cookie, kept across logouts (section 8.5, D-16) |
...
| `audit_log` | `id` | one plain table, no partitions; append-only for the application role (section 3.3, D-20); `payload` jsonb per the 9.7 payload rules |
...
`draft.draft_document` (`id` PK; `state` enum DraftState; `lease_expires_at`; `corpus_version_id` FK public.corpus_version; `opl_id_reserved` unique; `previous_draft_id` FK self; `session_scope` FK public.sandbox nullable, the frozen name holding the sandbox id, D-16), `draft.draft_field` (`id` PK; FK draft; `provenance`, `numeric_provenance` jsonb; CHECK `(is_slot AND text = 'REQUIRES ENGINEER INPUT') OR (NOT is_slot AND provenance IS NOT NULL)`), `draft.redline_verdict` ((`draft_id`, `round`) PK; `round` CHECK in (1, 2)), `draft.draft_transition` (`id` PK; FK draft; CHECK over the allowed pairs, below), `draft.sme_note` (`id` PK; FK draft, field; `provenance` CHECK = `'human, dated, unreviewed'`; `citeable` default false).
...
### 3.3 `audit_log`, append-only at the grant level (9.7, AC-NFR-07, D-20)

One plain table, no partitions. The application never connects as the owner: a dedicated role `thehub_app` is created in SQL by the owner role (never through the Neon API, whose roles carry `neon_superuser`) by `scripts/db/app-role.ts` (`pnpm db:app-role`) at T2, over `DATABASE_URL_UNPOOLED`, with a password the script reads from the env file (secret-blind, ADR-012; the variable is listed in `.env.example` with a placeholder). The grants, in this order: `USAGE` on the schemas `public` and `draft`; `SELECT`, `INSERT`, `UPDATE`, `DELETE` on every table of both; then `UPDATE` and `DELETE` revoked on `public.audit_log`. The script is re-run after any migration that adds a table. No migration carries a `REVOKE` (the former grants migration is gone). AC-NFR-07's test connects as `thehub_app`, attempts an UPDATE and a DELETE on `audit_log` and expects the database to refuse each.

Retention: the nightly job (section 10), as the owner over the unpooled URL (`scripts/db/retention.ts`, `pnpm db:retention`), deletes general events whose `server_ts` is older than 30 days and never the two safety actions `safety.request_refused` and `safety.request_served`, which are retained for the deployment's life; the application role holds no DELETE on the table, so nothing on a request path can do this. Reading the safety events writes `audit.safety_events_read` (9.7).

The application's `DATABASE_URL` is the pooled URL of `thehub_app`, assembled by shell from the owner URL (no model thread reads a value, ADR-012); `DATABASE_URL_UNPOOLED` (the owner) serves migrations, the seed, the app-role script and retention.

### 3.4 Sessions, rate limits, leases and versions in Postgres (AC-NFR-18)

No in-memory state is load-bearing: sessions in `session`, limits in `rate_limit_counter` (upsert `ON CONFLICT DO UPDATE SET count = count + 1 RETURNING count`; keys `ask:<user_id>` 30 per minute, `draft:<user_id>` 5 per minute, `addr:<ip>` 120 per minute; a hit above the limit renders the designed 429 naming the limit and `window_start + 60 s`), draft leases in `draft.draft_document.lease_expires_at`, the sandbox in `sandbox` (named by the `thehub_sandbox` cookie, D-16), warm caches only as optimisation. Revision labels order as the drawings are issued: a numeric label supersedes any alphabetic one (A and B are pre-issue, 0 is the first issue), numerics compare as numbers and letters alphabetically; `revision_date` is free sheet text and never orders. Lineage rule applied by activation: for the activated version V, `document_revision.is_current` is true for the latest revision per document whose `corpus_version_id` is V or an ancestor of V, false for every revision outside that lineage; rows are never deleted (AC-ING-10 `activate_v1_after_publish`).
...
| --- | --- |
| accounts | `app_user` rows seeded by `pnpm db:seed` from environment: `DEMO_ENGINEER_PASSWORD`, `DEMO_SUPERVISOR_PASSWORD`, `DEMO_MANAGER_PASSWORD` (roles Engineer, Reviewing Supervisor, Manager; `is_demo = true`) and `ADMIN_PASSWORD` (Admin, `is_demo = false`, never distributed); usernames `engineer_demo`, `supervisor_demo`, `manager_demo` and `admin`, as printed on the credentials sheet; aliases are role aliases, never personal names |
...
| login | `POST /api/auth/login { username, password }` (Zod) -> `session` row (`id` = 32 random bytes base64url, `expires_at` = now + 8 h) -> cookie `thehub_session = <id>.<hmac_sha256(id, AUTH_SECRET)>`, `HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=28800`; rotating `AUTH_SECRET` invalidates every cookie; failures are uniform (no username enumeration) and rate-limited under `addr:<ip>`; the same response issues `thehub_sandbox` (32 random bytes, `HttpOnly; Secure; SameSite=Lax`, 30 days) when the browser has none and inserts the `sandbox` row (section 8.5, D-16); a login carrying `next=/demo/loop` with the username prefilled is the guided route's role switch |
| logout | `POST /api/auth/logout` deletes the row and clears `thehub_session`; `thehub_sandbox` is kept (D-16); `GET /api/auth/session` returns `{ alias, role, expires_at }` or 401 |
...
| landing | after login the user lands on `/tour` (D-07), which ends on `/demo/loop`; a login with `next=/demo/loop` (the role switch of section 8.5, D-16) lands there directly; the Reviewer mode banner is not rendered anywhere (no reviewer sessions exist) |
...
9. The one evidence set (`src/answer/evidence.ts`): the retrieved chunk citations in rerank order followed by the source citation of every typed fact and every block item, each with the anchor text of its span, deduplicated by `span_id`. Line 1 of the stream stays the retrieved set alone (9.8), while the composer, the verifier and C1 all read this set, so a sentence that states a typed fact cites a span the gate can resolve and the verifier can read.
10. AG-2 compose (section 9): envelope `{ question, template, scope, evidence, typed_facts, repair: null }` where `evidence` is that set; output `{ claims, gaps, suggested_outcome }`.
11. AG-4 verify (question-blind): `{ pairs: [{ sentence_id, sentence, spans }] }` with each span's text taken from the same set, one batched call; a parse failure is `not_entailed`.
12. G2 (`src/gates/g2`): C1 citation resolution against that set, C2 quote fidelity (hash recomputed in the canonical form), C3 numeric fidelity (every numeral typed with unit in a cited source row, else the sentence goes), C4 approval filter, C5 rule-pack clearance of the outbound text with the approved-lesson spans of that set whitelisted first (a whitelist of chunk texts never matches a claim that quotes the anchor text inside one, and the quotation would screen as a defeat, which INV-2 calls a safety failure) whitelisted first (`screen.ts`, AC-ANS-17), C6 entailment from the verdicts; the gate decides, the verifier never does.
13. Repair once (AC-ANS-19): if C6 or C3 dropped a sentence, AG-2 is called again with `repair: { verdicts }` and may only drop or reword the named sentences; then AG-4 and G2 run again; no third attempt; `repair_rounds` 0 or 1.
...
15. Persist `answer_trace` (immutable), `gateway_call` rows, `audit_log` (`answer.issued` or `answer.abstained`, never the question text or a span), then line 2 `{ stage: "packet", packet }`, then close.
...
`src/coverage/` ports the frozen recipe of 9.5 field by field: `tokenise.ts` (lower-cased alphanumerics, tag-like tokens kept whole, single characters dropped, the 65-word stop list read from `fixtures.method.stop_list` and checked against `fixtures.method.stop_list_sha256` at load; both lanes read the fixture and no `bundle/coverage/stop_list.txt` exists, D-19), `window.ts` (windows of twice the field's content-word count advancing one token, largest share of distinct content words in one window), `layers.ts` (generous over whole lesson text, strict over header fields and sections 1, 2, 3, 4, 6 cut at the watermark), `summary.ts` (populations, bands, sensitivity ladder), `debt.ts` (0.4 D/Dmax + 0.3 C/Cmax + 0.2 k + 0.1 r, k by criticality, r by family share, `incomplete_uncovered` beside the score). The equality gate is a Vitest test that runs the port over the seeded corpus inputs and compares `coverage_scores`, both summaries at every t of the ladder and `debt.per_asset` byte for byte (canonical JSON) with `bundle/coverage_scores.json`, `bundle/fixtures.json` and `bundle/debt.json`; a divergence fails the build. The rule-pack equality test (AC-ANS-10) does the same over `rulepack.fixtures` from both entry points.
...
### 8.5 Sandbox per browser (blueprint section 2, AC-LOOP-13, NFR-17, D-16)

Blueprint line 50 scopes the sandbox to the visitor's session, but the 9.9 matrix needs the Reviewing Supervisor (request, accept) and the Manager (publish) to walk the loop, so one login cannot complete it and a session-keyed sandbox would vanish at each role switch. The sandbox is therefore scoped to the visitor's browser (D-16): the first login issues the `thehub_sandbox` cookie (32 random bytes, HttpOnly, Secure, SameSite=Lax, 30 days) and inserts the `sandbox` row (`id`, `corpus_version_id` null, `created_at`, `last_seen_at`, the last refreshed at each login); logout keeps the cookie, so the next login in the same browser resumes the same sandbox. Every draft created through the product by a demo account carries `session_scope = sandbox.id` (the frozen field name, section 3.2); the queue and detail routes show `session_scope IS NULL OR session_scope = current sandbox`. Seeded drafts from `bundle/seeded/drafts.json` carry `session_scope = null` and are replay sources: a `POST /api/drafts` for a cluster that has a seeded draft on the seeded corpus version clones it into the sandbox (fields, verdict, transitions replayed proposed -> drafted -> redlined -> in_review) with zero provider calls, which is the guided route's "request a lesson" (NFR-17: the guided loop route renders stored output; AC-NFR-15); a cluster without a seeded draft runs AG-3 live under the cap. Publication from a sandbox creates a child `corpus_version` that is never activated and writes its id into `sandbox.corpus_version_id`; `visibleVersionIds(sandbox)` = the active version's lineage plus the sandbox's version, used by retrieval, the Console, the version badge and the recount, so one visitor's publication never reaches another's numbers. The guided route names the role each step needs (Reviewing Supervisor: request and accept; Manager: publish; any role: ask) and offers a role switch: logout, then login with the username prefilled and `next=/demo/loop`, back on the route inside the same sandbox; every action runs under `authorize()` and the 9.9 matrix, so INV-3 holds literally (only the Manager publishes). Admin cannot publish and the Manager demo account is the only publisher, so every publication in this deployment is sandboxed; `activation.ts` (nightly) re-asserts the seeded version and applies the lineage rule of section 3.4, deleting nothing.
...
4. Create the child `corpus_version` (`parent_version_id` = the sandbox's visible version of section 8.5, `label` = next `v<n>`, `is_active = false`, pins copied), `coverage_method` copied, `coverage_assessment`, `coverage_summary` and `debt_cluster` recomputed by the port of section 8.1 for the new version (both layers, same `recipe_sha256` and `stop_list_sha256` as the baseline).
5. Flip `sme_note.citeable = true`; `transition(accepted -> published, Manager)`; `audit_log` `draft.published` with the version id; `sandbox.corpus_version_id` set to the new version (D-16).
...
AG-1 runs through this module too (D-18): `scripts/ag1/extract.ts` (`pnpm ag1:extract`) is the extractor role's only entry point, calling `invoke("AG-1", envelope, schema)` under the role table of 9.2, reading the harness's extracted text from a local path (never tracked) and writing candidate claims into `thehub-harness/packages/` for review and commit. No Python provider client exists, `harness/ag1.py` is not built and `ZAI_API_KEY` is not in the harness env example. The extraction pass runs once this gateway exists (T3); the bundle carries the reviewed claims from that pass.
...
| piece | configuration |
| --- | --- |
| Vercel Hobby, fluid compute | project wired to `finerium/thehub-3v`: production from `main`, previews from pull requests; the build command is `pnpm vercel-build` = `drizzle-kit migrate` over the unpooled owner URL, then `pnpm models:fetch` (the three pinned embedder files under a hash check), then `next build`, so a deployment can never precede its schema and the model files sit inside the traced function; `next.config.ts` traces `models/**` and the linux-x64 `onnxruntime-node` binary into `/api/ask` and `/api/search` only and excludes every other platform's binary; `vercel.json` `{ "regions": ["sin1"] }`; `maxDuration` set explicitly per route (`api/ask` 120, `api/drafts` 300, everything else default); verified limits: 300 s default and maximum, 2 GB memory, 4.5 MB body, 250 MB uncompressed function; `next.config.ts` sends `X-Robots-Tag: noindex, nofollow, noarchive, nosnippet` and the NFR-11 baseline headers on every response; `public/robots.txt` is `User-agent: *` / `Disallow: /` |
| Neon Free, `sin1`, pgvector | provisioned by the Orchestrator through `vercel install neon` (ADR-012); the integration writes the owner URLs, and `DATABASE_URL` for the application and the harness is the pooled URL of the `thehub_app` role, assembled by shell from the owner URL, while `DATABASE_URL_UNPOOLED` (the owner) serves migrations, the seed, the app-role script and retention (D-20, section 3.3); Neon Free (D-06) allows 100 CU-hours per project per month and scale-to-zero after 5 minutes cannot be disabled (verified, D-15), so the keep-alive is the cold-start mitigation at the D-15 cadence and the first request after an idle gap pays the Neon wake of about one second, inside the 3 s p95 target (AC-NFR-05); the seed for production runs from the build machine with dotenv (secret-blind) |
| secrets | `.env.example` lists every 9.15 name with a placeholder, plus the `thehub_app` password (D-20); values move only through `thehub/tools/secret-pipe.sh` into `vercel env add` and `gh secret set`; `ZAI_API_KEY` belongs to `thehub-3v` and its workflows only and is not in the harness env example (D-18); gitleaks on every push and on history (AC-FND-06, AC-NFR-20) |
| GitHub Actions `tier-a.yml` | on pull requests and pushes to `main`: `pnpm install --frozen-lockfile`, `gate:quick`, `contracts:check`, `scripts/audits/*` (provider egress, draft isolation, mock-pattern sweep, banned strings, fixed wordings, fonts, no-corpus-text, product-name grep, copy audit), gitleaks, `uv sync --frozen` + micromamba `poppler=26.02.0` + `make test` + the extractor-string assertion, the corpus checkout by deploy key, `make chunks pages bundle` (the seed-time `chunks.jsonl`, `pages/` and `opls.json`, D-17) into a `pgvector/pgvector:pg17` service container, `db:migrate && db:app-role && db:seed` (timed twice for AC-ING-15), the coverage and rule-pack equality tests, `golden:a` against `next start` on the runner with results posted to the runner's own app under `CI_INGEST_TOKEN`, and `golden:b:replay` from `recordings/` |
...
| `keepalive.yml` | cron `*/10 * * * *` until 2026-11-07 (the step exits when the date is past): every run `curl`s `/login` on the production URL (function warmth, no database) and the runs at minute 0 and 30 also `curl` `/api/health` (SELECT 1 and the active version; the route warms the embedder, section 6), so the database is touched every 30 minutes (D-15: a database ping every 10 minutes would consume Neon Free's monthly allowance; AC-NFR-05's 10-minute cadence is kept for the function and the database cadence is the deviation; D-06 primary mitigation) |
| `nightly.yml` | cron at 17:00 UTC (00:00 Asia/Jakarta): `POST /api/admin/corpus/activate { version_id }` with `ADMIN_JOB_TOKEN`, the id from `scripts/seeded-version-id.ts` over `bundle/manifest.json`; then the audit retention of section 3.3 (`pnpm db:retention` as the owner over `DATABASE_URL_UNPOOLED`: general events older than 30 days deleted, the two safety actions never, D-20) and `rate_limit_counter` rows older than one hour deleted by the same `pnpm db:retention` step |
| `seed.yml` | `workflow_dispatch` only: the reproducible seed (corpus by deploy key, `make bundle`, G1, `db:seed`) against a named owner (unpooled) database URL secret (D-20); used for recovery, never on a schedule |
...
| T1 Harness and packages | corpus, extractor pin | builder A: `harness/` adoption and the bundle writer (`opls.json` as a seed-time file, D-17), `make chunks pages embed bundle release`; builder B (one hand): `rulepack/v1.json` reshaped to 9.10; builder C: `golden/cases.yaml` tiers and the 9.11 fields; the AG-1 claims are reviewed and committed into `packages/` once T3's gateway has run `pnpm ag1:extract` (D-18) | test-author: `tests/` (fixture reproduction, spans, G1 mutations, rule-pack fixture counts, canonical cases) | Reproduction fleet (one agent per Appendix A group of 10.4); Extraction review; Sidecar review (one agent per set, D-12 labels) | `make test` green, bundle released, fixture keys of 10.5 present |
| T2 Data layer | T0 contracts; bundle format | builder: `src/db/schema.ts`, migrations (section 3), `db:seed` with G1, page derivatives, corpus versioning and activation, the `sandbox` table (D-16), `scripts/db/app-role.ts` and `scripts/db/retention.ts` (D-20) | test-author: G1 mutation tests, `activate_v1_after_publish`, constraint-violation tests (AC-NFR-13), audit revoke test connecting as `thehub_app` (AC-NFR-07, D-20), `db:reset` timing | Contracts review on the Drizzle schema | seed of corpus version v1 on production; M1 smoke rows |
| T3 Answer engine core | T2 schema, rule pack file | builder A: `src/gateway/` (section 9) and `scripts/ag1/extract.ts` (D-18); builder B (one hand): `src/rulepack/` port and its suppressions; builder C: `src/answer/` (scope, retrieval, templates, permit, confidence, stream, seeded), `src/gates/g2`, prompts AG-2 and AG-4/verify | test-author: gate tests first (C1 to C6, 100 percent lines), rule-pack equality, recorded-call tests (no question in AG-4), gateway call counter, fault injection (AC-NFR-19) | Rule-pack red team (both languages; blocker: a defeat served or a documented procedure refused) | AC-ANS-08 and AC-EVAL-04 hard gates at 16 of 16; M1 `POST /api/ask` live on UC-1 |
...
| T7 Deliverables | frozen M2 deployment, fixture, team facts | builder A (one hand): deck copy voice and `deck/`; builder B: `video/`; builder C: README with the inter-repo SVG, `docs/runbook.md`, `docs/DISCLOSURE-AI.md`, CHANGELOG | test-author: presubmit fixtures (inject a placeholder, expect red) | Deck compliance; Critique fleet (five to seven lenses) on the tour, the loop route and the deck; README and hygiene review per repository | AC-DEL-02 to 08; `checkpoint(release)` |
...
- M0 (AC-M0-01 to 06): `/api/health` 200 with `SELECT 1`, the active version and the commit; `/` after login renders the corpus status card from the seeded equipment master; `/robots.txt` is `Disallow: /`; `X-Robots-Tag` on every response; a designed 404; `keepalive.yml` present and running at the D-15 cadence (`/login` every 10 minutes, `/api/health` every 30 minutes); Tier A green on a pull request.
...
Deviations applied here: D-01 with D-23 (one high-capability model at effort max for the main thread and every worker, named by the owner and changed when an allowance runs out; the model policy of blueprint 13 is superseded), D-02 (control plane at the ChandraAsri-Competition root), D-03 (author identity string), D-04 (no trailers; DISCLOSURE-AI.md), D-05 (glm-5.3-flash for AG-1 to AG-4; section 9.2), D-06 (Hobby and Free plans; keep-alive as the primary mitigation; section 10), D-07 (fully behind login; section 5; the export carries the live URL only), D-08 (supervisor line and presubmit check 6; section 11), D-09 (silent audio with burned captions), D-10 (three repositories, contracts inside the harness; section 1), D-11 (pdftotext 26.02.0 through micromamba; section 4), D-12 (adopted sidecars and hand-verified readings as `agent_transcription`, `pending`; section 3.1), D-13 (worker kinds; section 12), D-14 (agent kinds registered after one session restart; process only, nothing here changes), D-15 (keep-alive `/login` every 10 minutes and `/api/health` every 30 minutes under Neon Free's allowance; sections 6 and 10), D-16 (sandbox scoped to the browser through `thehub_sandbox` and the `sandbox` table; sections 3, 5 and 8.5), D-17 (`opls.json` is a seed-time artefact, never released or tracked; sections 1 and 2), D-18 (AG-1 as `pnpm ag1:extract` over the one gateway, no Python provider client; sections 1, 9.1 and 12), D-19 (the stop list in `fixtures.method.stop_list`; section 8.1), D-20 (the `thehub_app` role, no partitions, retention by the owner's nightly DELETE; sections 3.3 and 10).
...
2. Browser sandbox (D-16): the `sandbox` table, the `thehub_sandbox` cookie and the never-activated child version (section 8.5) implement AC-LOOP-13 without touching a frozen type (`session_scope` keeps its name and holds the sandbox id); the lineage rule of section 3.4 defines `is_current` after activation.
...
7. Audit retention by the owner's nightly DELETE of general events older than 30 days, never the two safety actions, with UPDATE and DELETE revoked from the application role `thehub_app` and no partitions (section 3.3, D-20).
...
12. `GET /api/search` and line 1 of the ask stream return the same shape `{ trace_id, corpus_version, scope, evidence }` (plus `hits` and a refusal where one applies) rather than 9.9's `{ chunks, documents }` (D-22): one evidence shape for both entry points, and chunk text never leaves as text.
13. Recordings are dehydrated to span and chunk ids plus quote hashes (section 9.4); seeded drafts are cloned into the sandbox on `POST /api/drafts` for clusters that have one.
...
- The AG-1 extraction pass (`pnpm ag1:extract`, D-18) runs once the T3 gateway exists; its reviewed claims are committed into `thehub-harness/packages/` and the bundle is released from them, so the bundle carries the reviewed claims from that pass.
- The Z.ai price constants for the spend cap and the dated terms reading belong to ADR-001 (human-gated line for the terms).
