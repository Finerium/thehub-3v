...
12. Recordings are dehydrated to span and chunk ids plus quote hashes (section 9.4); seeded drafts are cloned into the sandbox on `POST /api/drafts` for clusters that have one.
