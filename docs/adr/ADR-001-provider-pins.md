...
| AG-2 Composer | AG-2 | `glm-5.3-flash` | low | json_object | 0 | 4096 | 35000 | `prompts/AG-2/v3.md` | 3,000,000 | 20,000 |
| AG-3 Drafter | AG-3 | `glm-5.3-flash` | high | json_object | 0 | 16384 | 150000 (retries 0, D-27) | `prompts/AG-3/v2.md` | 3,000,000 | 20,000 |
| AG-4 Verifier | AG-4 | `glm-5.3-flash` | low | json_object | 0 | 2048 | 20000 | `prompts/AG-4/verify/v2.md` | 3,000,000 (shared) | 20,000 (shared) |
| AG-4 Redliner | AG-4 | `glm-5.3-flash` | low | json_object | 0 | 2048 | 60000 (retries 0, D-27) | `prompts/AG-4/redline/v1.md` | shared with the verifier | shared with the verifier |
...
- The rows above are the pins as `src/gateway/config.ts` carries them on 2026-09-13. Three moved after the first reading: AG-2 to `4096 / 35000 / prompts/AG-2/v3.md` (v3 keeps every rule of v2 and adds the yield rules), AG-3 as the dated note below records and then to `150000` with its gateway retries given up (D-27, measured on the deployment's own rows), and AG-4 to `prompts/AG-4/verify/v2.md`, which weighs a sentence against its spans together (9.8). The earlier files stay in the tree as history.
...
- Confirmation run AC-EVAL-05, taken 2026-09-13 (`pnpm eval:confirm`, `scripts/eval/confirm-entailment.ts`): one batched, question-blind AG-4 call over the 30 labelled entailment pairs of `tests/unit/entailment.ts`, 15 supported and 15 unsupported, on `glm-5.3-flash` with the verifier prompt `prompts/AG-4/verify/v2.md`, prompt version `b84e50e3a9a5fd921a1115be94d1955bb42dd193d05b45458a98b5f7f0038d25`. It returned `ok` in one `gateway_call` row of 2972 input and 1273 output tokens with 30 verdicts, 15 `entailed` and 15 `not_entailed` and none `contradicted`, which is 30 agreements of 30 with no false accept and no false reject against an acceptance of at least 27 agreements and at most one false accept: the confirmation passes.
